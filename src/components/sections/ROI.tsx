        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, i) => (
            <motion.div key={stat.key} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              className="glass-card rounded-2xl p-8 text-center hover:shadow-xl hover:shadow-brand-500/10 transition-all duration-300 hover:-translate-y-1"
            >
              ...
              <div className="text-surface-400 font-medium text-sm">{t(stat.key)}</div>
            </motion.div>
          ))}
        </div>
      </div>
