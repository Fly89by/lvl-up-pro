  ...
  return (
    <section className="py-12 bg-zinc-50 border-y border-zinc-100">
      <div className="container-main">
        <p className="text-xs font-medium text-zinc-400 text-center uppercase tracking-widest mb-8">
          {t("title")}
        </p>
          ...
          <div className="flex gap-16 animate-[scroll_30s_linear_infinite] min-w-full shrink-0">
            {logos.map((i) => (
              <div key={`a-${i}`} className="flex items-center justify-center h-8 w-24 shrink-0">
                <div className="w-full h-full rounded bg-zinc-200/50 flex items-center justify-center">
                  <span className="text-xs text-zinc-400 font-medium">Logo {i}</span>
                </div>
              </div>
            ))}
          </div>
