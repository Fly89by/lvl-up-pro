...
export default function SettingsPage() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="text-xl font-bold text-surface-900 animate-fade-in-up">{t("settings")}</h1>
        <p className="text-surface-500 text-sm mt-1">Personal Account Settings</p>
      </div>
      ...
      <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-surface-100 shadow-sm p-6 glass-card">
        <div className="flex items-center gap-4 pb-6 border-b border-surface-100">
          ...
          <div>
            <h2 className="text-lg font-semibold text-surface-900">{fullName}</h2>
        ...
        <form onSubmit={handleSave} className="space-y-5 pt-6">
          ...
          <div>
            ...
            <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)}
              className="w-full px-4 py-2.5 rounded-xl border border-surface-200 focus:border-brand-400 focus:ring-2 focus:ring-brand-200 outline-none" dir="ltr" />
          </div>
          <div>
        ...
        <div className="mt-6 pt-6 border-t border-surface-100">
          <h3 className="text-sm font-bold text-surface-900 mb-3">AI Integration</h3>
          <div className="bg-gradient-to-br from-purple-50 to-brand-50 rounded-2xl border border-purple-100 p-4">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-2 h-2 rounded-full bg-emerald-500" />
              <span className="text-sm font-medium text-surface-800">Nemotron 3 Ultra</span>
              <span className="text-[10px] text-surface-400 bg-white px-1.5 py-0.5 rounded">Free Tier</span>
            </div>
            <p className="text-xs text-surface-600">AI analysis, recommendations, and smart assistant are active and powered by Nemotron 3 Ultra via OpenRouter.</p>
          </div>
        </div>
      </div>
    </div>
