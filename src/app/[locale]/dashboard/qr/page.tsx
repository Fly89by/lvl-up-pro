...
export default function QRScannerPage() {
  ...
  return (
    <div className="max-w-md mx-auto space-y-6">
      <div>
        <h1 className="text-xl font-bold text-zinc-900">Scan QR Code</h1>
        <p className="text-zinc-500 text-sm mt-1">Scan a QR code for quick access to inspection templates</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm p-6 space-y-4">
        <div className="relative aspect-video bg-zinc-900 rounded-xl overflow-hidden flex items-center justify-center">
          {scanning ? (
            ...
            <QrCode className="w-16 h-16 text-zinc-500" />
          )}
        </div>
        ...
        <div className="flex justify-center gap-3">
          {!scanning ? (
            ...
            <button onClick={stopScanning}
              className="flex items-center gap-2 px-6 py-2.5 bg-red-600 text-white rounded-xl text-sm font-medium hover:bg-red-700">
      ...
      <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm p-6">
        <h2 className="text-sm font-semibold text-zinc-900 mb-3">Or enter template code manually</h2>
        <form onSubmit={handleManualSubmit} className="flex gap-2">
          <input type="text" value={result || ""} onChange={(e) => setResult(e.target.value)}
            placeholder="Template ID..."
            className="flex-1 px-4 py-2.5 rounded-xl border border-zinc-200 text-sm focus:border-brand-400 outline-none" />
          <button type="submit"
