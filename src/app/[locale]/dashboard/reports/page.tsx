  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-surface-900 animate-fade-in-up">{t("reports")}</h1>
          <p className="text-surface-500 text-sm mt-1">Performance Reports & Analytics</p>
        </div>
      </div>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
              activeReport === tab.key
                ...
                : "bg-white text-surface-600 border border-surface-200 hover:border-brand-200"
            }`}
          >
            {tab.labelAr}
          </button>
      ...
      {loading ? (
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-surface-100 shadow-sm p-6 glass-card">
          <div className="text-center mb-6">
            <p className="text-3xl font-bold text-surface-900">{data.total}</p>
            <p className="text-sm text-surface-500 mt-1">Total {reportTabs.find((t) => t.key === activeReport)?.labelAr}</p>
          </div>

          <div className="space-y-3">
            {data.labels.length === 0 && (
              <p className="text-center py-8 text-surface-400 text-sm">{t("noData")}</p>
            )}
              ...
              return (
                <div key={label}>
                  <div className="flex items-center justify-between text-sm mb-1">
                    ...
                    <span className="text-surface-500">{data.values[idx]}</span>
                  </div>
      ...
      {aiAnalysis && (
        <div className="mt-6 bg-gradient-to-br from-purple-50 to-brand-50 rounded-2xl border border-purple-100 p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setAiAnalysis(null)} className="text-surface-400 hover:text-surface-600">
              <X className="w-4 h-4" />
            </button>
          </div>
          {aiAnalysis.summary && <p className="text-sm text-surface-700">{aiAnalysis.summary}</p>}
          {aiAnalysis.key_insights?.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-surface-600 mb-1">Key Insights</p>
              <ul className="space-y-1">{aiAnalysis.key_insights.map((ins: string, i: number) => <li key={i} className="text-sm text-surface-700 flex gap-2"><span className="text-purple-500">•</span>{ins}</li>)}</ul>
            </div>
          )}
          {aiAnalysis.recommendations?.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-surface-600 mb-1">Recommendations</p>
              <ul className="space-y-1">{aiAnalysis.recommendations.map((rec: string, i: number) => <li key={i} className="text-sm text-surface-700 flex gap-2"><span className="text-brand-500">→</span>{rec}</li>)}</ul>
            </div>
          )}
        </div>
      )}
