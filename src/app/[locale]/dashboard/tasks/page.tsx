  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-surface-900 animate-fade-in-up">{t("tasks")}</h1>
          <p className="text-surface-500 text-sm mt-1">Task Management</p>
        </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl w-full max-w-lg p-6 space-y-4 glass-card" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-surface-900">New Task</h2>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })}
                    className="w-full px-3 py-2 rounded-xl border border-surface-200 text-sm outline-none">
      ...
      <div className="space-y-2">
        {tasks.length === 0 && <p className="text-center py-12 text-surface-400 text-sm">{t("noData")}</p>}
        {tasks.map((task) => (
          <div key={task.id} className="bg-white/80 backdrop-blur-xl rounded-2xl border border-surface-100 shadow-sm p-4 flex items-start gap-3 glass-card">
            <button onClick={() => toggleStatus(task)} className="mt-0.5">
              {task.status === "completed" ? (
                <CheckCircle2 className="w-5 h-5 text-emerald-500" />
              ) : (
                <div className="w-5 h-5 rounded-full border-2 border-surface-300 hover:border-brand-400 transition-colors" />
              )}
            </button>
            <div className="flex-1 min-w-0">
              <p className={`text-sm ${task.status === "completed" ? "text-surface-400 line-through" : "text-surface-900 font-medium"}`}>
                {task.title}
              </p>
              {task.description && <p className="text-xs text-surface-500 mt-0.5">{task.description}</p>}
              <div className="flex items-center gap-3 mt-2 flex-wrap">
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${priorityColors[task.priority]}`}>
                  {task.priority === "urgent" ? "Urgent" : task.priority === "high" ? "High" : task.priority === "medium" ? "Medium" : "Low"}
                </span>
                {task.branches?.name && <span className="text-xs text-surface-400">{task.branches.name}</span>}
                {task.due_date && (
